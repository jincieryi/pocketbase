<script>
    ...
    import DatasourceSelectOption from "./DatasourceSelectOption.svelte";
    ...
    export let optionComponent = DatasourceSelectOption; // custom component to use for each dropdown option item
    ...
    async function loadSelected() {
        ...
        try {
            ...
            selected = await ApiClient.records.getFullList("datasources",100, {
                sort: "-created",
                filter: filters.join("||"),
    ...
    async function loadList(reset = false) {
        ...
        try {
            ...
            const result = await ApiClient.records.getList("datasources",page, 200, {
                sort: "-created",
...
<ObjectSelect
        ...
        selectionKey="id"
        labelComponent={DatasourceSelectOption}
        {optionComponent}
        {multiple}
        bind:keyOfSelected
        bind:selected
